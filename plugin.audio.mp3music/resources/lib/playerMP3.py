...
def getListItem(title, artist, album, track, image, duration,
...
def fetchNext(posn):
    ...
    if not url.startswith('plugin://plugin.audio.mp3music'):
        return
...
def fetchFile(title, artist, album, track, url, filename):
    ...
    if ((xbmcvfs.exists(xbmc.translatePath(filename)) and
         xbmcvfs.File(filename).size() > 250 * 1024)):
...
class Downloader(threading.Thread):
        ...
        try:
            headers = {'Host': 'listen.musicmp3.ru',
                       ...
                       'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; '
                                     ...
                                     'Firefox/44.0', 'Accept': 'audio/'
                                     'webm,audio/ogg,audio/wav,audio/*;q='
                                     '0.9,application/ogg;q=0.7,video/*;q='
                                     '0.6,*/*;q=0.5',
                       'Referer': 'https://www.goldenmp3.ru'}
            xbmcgui.Window(10000).setProperty(PROPERTY % self.slot,
                                              'Downloading')
            ...
            with closing(requests.get(self.url,
                ...
                for chunk in r.iter_content(chunk_size=1024):
                    if ((xbmcgui.Window(10000).getProperty(PROPERTY %
                         self.slot) == 'Signal')):
    ...
    def run(self):
        if xbmcvfs.exists(self.filename):
            log('DOWNLOADER - %s %s already exists' %
